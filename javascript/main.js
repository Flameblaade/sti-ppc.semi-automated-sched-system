...
(function() {
    ...
    window.showUnitLimitErrorModal = function(message) {
        // Remove any existing unit limit error modals
        const existing = document.getElementById('unitLimitErrorModal');
        if (existing) {
            existing.remove();
        }
        
        const modal = document.createElement('div');
        modal.id = 'unitLimitErrorModal';
            ...
            ">
                ...
                ">
                    ...
                    <div style="flex: 1;">
                        <h3 style="
                            ...
                            font-size: 20px;
                            font-weight: 600;
                        ">Unit Limit Exceeded</h3>
                        <p style="
                            margin: 4px 0 0 0;
                            color: rgba(255, 255, 255, 0.9);
                            font-size: 14px;
                        ">Class creation blocked</p>
                    </div>
                    <button type="button" id="closeUnitLimitModal" style="
                        background: rgba(255, 255, 255, 0.2);
                        border: none;
                        color: white;
                        font-size: 24px;
                ...
                <div style="padding: 24px;">
                    ...
                    <p style="
                        ...
                        font-size: 14px;
                        line-height: 1.5;
                    "><strong>Action Required:</strong> Please reduce the unit load or select a different faculty member to proceed with class creation.</p>
                </div>
                <div style="
                    padding: 16px 24px;
                    ...
                    gap: 12px;
                ">
                    <button type="button" id="okUnitLimitModal" style="
                        background: linear-gradient(135deg, #ef4444, #dc2626);
                        color: white;
                        border: none;
                        padding: 10px 24px;
                    ...
                    " onmouseover="this.style.transform='translateY(-1px)'; this.style.boxShadow='0 4px 12px rgba(239, 68, 68, 0.4)'" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='none'">Understood</button>
                </div>
            </div>
        `;
        ...
        const closeBtn = modal.querySelector('#closeUnitLimitModal');
        const okBtn = modal.querySelector('#okUnitLimitModal');
        ...
        // Add CSS animations if not already present
        if (!document.getElementById('unitLimitModalStyles')) {
            const style = document.createElement('style');
            style.id = 'unitLimitModalStyles';
            style.textContent = `
                @keyframes fadeIn {
                    from { opacity: 0; }
                    to { opacity: 1; }
    ...
    window.showUnitOverrideModal = function(details, onConfirm, onCancel) {
        const existing = document.getElementById('unitOverrideModal');
        if (existing) {
            existing.remove();
        }
        
        const modal = document.createElement('div');
        modal.id = 'unitOverrideModal';
        modal.className = 'custom-modal';
        modal.style.cssText = `
            position: fixed;
            top: 0;
        ...
        modal.innerHTML = `
            ...
            ">
                ...
                ">
                    ...
                    <div style="flex: 1;">
                        <h3 style="margin: 0; font-size: 20px;">Allow Unit Overload?</h3>
                        <p style="margin: 4px 0 0 0; font-size: 14px;">${facultyName} (${facultyType}${mixedTeaching ? ', Mixed Teaching' : ''})</p>
                    </div>
                </div>
                <div style="padding: 24px; color: #374151;">
                    <p style="margin-top: 0; line-height: 1.5;">
                        This assignment will raise the unit load from <strong>${currentUnits.toFixed(1)}</strong> to 
                        <strong>${totalUnits.toFixed(1)}</strong>, exceeding the base limit of ${baseLimit} units but still within the
                        extended ceiling of ${absoluteLimit} units for approved overloads.
                    </p>
                    <div style="
                        background: #fff7ed;
                    ...
                    ">
                        <div>
                            <div style="color: #fb923c; font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.05em;">Current Units</div>
                            <div style="font-size: 1.3rem; font-weight: 700;">${currentUnits.toFixed(1)}</div>
                        </div>
                        <div>
                            <div style="color: #fb923c; font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.05em;">Adding</div>
                            <div style="font-size: 1.3rem; font-weight: 700;">+${newUnitLoad.toFixed(1)}</div>
                        </div>
                        <div>
                            <div style="color: #fb923c; font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.05em;">Total</div>
                            <div style="font-size: 1.3rem; font-weight: 700;">${totalUnits.toFixed(1)}</div>
                        </div>
                    </div>
                    <p style="margin: 18px 0 0 0; font-size: 0.9rem; color: #6b7280;">
                        Approving this will record the overload decision in the activity log and allow the superadmin to continue assigning the class.
                    </p>
                </div>
                <div style="
                    padding: 16px 24px;
                    ...
                    gap: 12px;
                ">
                    <button type="button" id="unitOverrideCancel" style="
                        ...
                        cursor: pointer;
                    ">Keep Limit</button>
                    <button type="button" id="unitOverrideConfirm" style="
                        border: none;
        ...
        modal.querySelector('#unitOverrideCancel')?.addEventListener('click', () => {
            closeModal();
            if (typeof onCancel === 'function') onCancel();
        });
        
        modal.querySelector('#unitOverrideConfirm')?.addEventListener('click', () => {
            closeModal();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (submitScheduleBtn) {
        submitScheduleBtn.addEventListener('click', async function(e) {
            ...
            // Get subject data first to get units for validation (prioritize reading from subjects)
            let subjectUnits = 0;
            let subject = null;
            try {
                ...
                // Get units from subject (prioritize reading from subjects)
                if (subject && subject.units) {
                    subjectUnits = parseFloat(subject.units) || 0;
                }
            } catch (e) {
                console.warn('Could not load subject data for validation:', e);
            }
            
            const finalizeClassCreation = () => {
                // Use subject data already fetched for validation to get lecture/lab hours
                let lectureHours = 0;
                let labHours = 0;
                let classType = 'lecture';
                let subjectCode = '';
                let subjectName = selectedSubjectName;
                let splitLectureHours = false;
                let splitLabHours = false;
                
                // Subject data was already fetched above, use it here
                if (subject) {
                    ...
                    splitLectureHours = !!subject.splitLectureHours;
                    splitLabHours = !!subject.splitLabHours;
                ...
                // subjectUnits was already set above during validation, just ensure it's set if subject was found later
                if (subject && subject.units && subjectUnits === 0) {
                    subjectUnits = parseFloat(subject.units) || 0;
                }
                ...
                // Adding a class invalidates any "cleared" flag
                localStorage.removeItem('classesCleared');
            ...
            // Use subject units if available, otherwise fall back to input value
            if (selectedFacultyId) {
                const unitLoadInput = document.getElementById('unitLoadInput');
                const inputUnitLoad = parseFloat(unitLoadInput?.value?.trim() || '3');
                // Prioritize subject units over input value
                const newUnitLoad = subjectUnits > 0 ? subjectUnits : inputUnitLoad;
                
                console.log('Validating faculty unit limit for faculty ID:', selectedFacultyId, 'with new unit load:', newUnitLoad, '(from subject:', subjectUnits > 0 ? 'yes' : 'no', ')');
                const validationResult = await validateFacultyUnitLimit(selectedFacultyId, newUnitLoad);
                ...
                if (!validationResult.allowed) {
                    if (validationResult.requiresApproval) {
                        ...
                        if (normalizedRole !== 'superadmin') {
                            showUnitLimitErrorModal(validationResult.message || 'Only superadmins can approve unit overloads.');
                            return;
                        }
                        
                        showUnitOverrideModal(
                            {
                                ...validationResult,
                                facultyName: validationResult.facultyName || selectedFacultyName
                            },
                            () => {
                    ...
                    showUnitLimitErrorModal(validationResult.message);
                    return;
                }
            }
        ...
        clearBtn.addEventListener('click', async function() {
            try {
                ...
                if (typeof clearAllClasses === 'function' && classesList) {
                    await clearAllClasses(classesList, classItems);
                } else {
        ...
        try {
            new FullCalendar.Draggable(element, {
                ...
                eventData: function(dragEl) {
                    ...
                    return {
                        id: uniqueInstanceId + '-' + Math.random().toString(36).substr(2, 9), // Ensure truly unique ID for each drag instance
                        title: dragTitle,
    ...
    async function clearAllClasses(classesList, classItems) {
        ...
        // Reset allClasses array FIRST
        allClasses = [];
        window.allClasses = [];
        
        // Clear from localStorage to prevent restoration - do this IMMEDIATELY and explicitly
        try {
            // Remove the items completely first
            localStorage.removeItem('allClasses');
            localStorage.removeItem('calendarEvents');
            
            // Then set empty arrays to ensure they're cleared
            localStorage.setItem('allClasses', JSON.stringify([]));
            localStorage.setItem('calendarEvents', JSON.stringify([]));
            
            // Also set a flag to indicate classes were explicitly cleared
            localStorage.setItem('classesCleared', Date.now().toString());
            
            console.log('localStorage cleared - allClasses set to empty array');
            
            // Verify the clear worked
            const verify = JSON.parse(localStorage.getItem('allClasses') || '[]');
            if (verify.length > 0) {
                console.error('ERROR: localStorage still has classes after clear!', verify);
                // Force clear again
                localStorage.removeItem('allClasses');
                localStorage.setItem('allClasses', JSON.stringify([]));
            }
        } catch (e) {
            console.error('Error clearing allClasses from localStorage:', e);
            // Try again with a simpler approach
            try {
                localStorage.removeItem('allClasses');
                localStorage.setItem('allClasses', '[]');
                localStorage.setItem('classesCleared', Date.now().toString());
            } catch (e2) {
                console.error('Failed to clear localStorage even with fallback:', e2);
            }
        }
        ...
        // Persist and update UI state - CRITICAL: Save empty array to localStorage
        if (typeof saveClassesToLocalStorage === 'function') {
            try { 
                saveClassesToLocalStorage(); 
                ...
                const verify = JSON.parse(localStorage.getItem('allClasses') || '[]');
                if (verify.length > 0) {
                    console.error('WARNING: saveClassesToLocalStorage did not clear classes! Forcing clear...');
                    localStorage.setItem('allClasses', JSON.stringify([]));
                    window.allClasses = [];
                    allClasses = [];
                }
            } catch (e) { 
                console.error('saveClassesToLocalStorage failed:', e);
                // Force clear on error
                localStorage.setItem('allClasses', JSON.stringify([]));
            }
        } else {
            // Fallback: directly save empty array
            localStorage.setItem('allClasses', JSON.stringify([]));
        }
        
        // Leave a marker so reloads know classes were explicitly cleared
        localStorage.setItem('classesCleared', Date.now().toString());
        if (typeof updateClassesCountBadge === 'function') {
            try { updateClassesCountBadge(); } catch (e) { console.warn('updateClassesCountBadge failed', e); }
        }
    ...
    const MIN_SPLIT_HOURS = 2;
    const SPLIT_SESSION_COUNT = 2;
    const SPLIT_MIN_GAP_DAYS = 2;
    ...
    function shouldSplitHours(hours, enabled) {
        return enabled && hours >= MIN_SPLIT_HOURS;
    }
    
    function getSchedulingSegmentCount(classItem) {
        ...
        if (lectureHours > 0) {
            count += shouldSplitHours(lectureHours, classItem.splitLectureHours) ? SPLIT_SESSION_COUNT : 1;
        }
        
        if (labHours > 0) {
            count += shouldSplitHours(labHours, classItem.splitLabHours) ? SPLIT_SESSION_COUNT : 1;
        }
    ...
    function createSessionsForClassType(baseClass, type) {
        if (!baseClass) return [];
        const isLecture = type === 'lecture';
        const hours = isLecture ? normalizeHours(baseClass.lectureHours) : normalizeHours(baseClass.labHours);
        
        if (hours <= 0) {
            return [];
        }
        
        const splitEnabled = shouldSplitHours(hours, isLecture ? baseClass.splitLectureHours : baseClass.splitLabHours);
        const sessionCount = splitEnabled ? SPLIT_SESSION_COUNT : 1;
        const sessionDuration = hours / sessionCount;
        
        const sessions = [];
        for (let i = 0; i < sessionCount; i++) {
            sessions.push({
                ...baseClass,
                id: `${baseClass.id}-${type}-part-${i + 1}`,
                unitLoad: sessionDuration,
                splitSessionIndex: i + 1,
                splitSessionTotal: sessionCount,
                splitSessionType: type,
                splitSessionEnabled: splitEnabled
            });
        }
        ...
        setTimeout(function() {
            try {
                ...
                const incrementProgress = (message) => {
                    classesProcessed++;
                    const progressPct = Math.floor((classesProcessed / totalClasses) * 100);
                    if (progressBar) {
                ...
                const scheduleSegments = (segments, firstContext, options = {}) => {
                    ...
                    for (let i = 0; i < segments.length; i++) {
                        ...
                        const nextCount = classesProcessed + 1;
                        const labelBuilder = options.labelBuilder;
                        const label = typeof labelBuilder === 'function'
                            ? labelBuilder(segment, i, segments.length, nextCount, totalClasses)
                            : `Scheduling class ${nextCount} of ${totalClasses}: ${segment.subject}`;
                        
                        incrementProgress(label);
                        
                        let context = null;
                        if (i === 0) {
                            context = sanitizeContext(firstContext);
                        } else {
                            const previousResult = results[i - 1] || {};
                            context = sanitizeContext({
                                time: results[0]?.time || firstContext?.time || null,
                                day: previousResult.day || firstContext?.day || null,
                                excludeDay: previousResult.day || null,
                                minGapDays: (segment.splitSessionEnabled && segments.length > 1) ? SPLIT_MIN_GAP_DAYS : undefined
                            });
                        }
                        
                        const scheduled = scheduleClass(segment, context);
                        if (scheduled && scheduled !== true && scheduled.success) {
                            results.push(scheduled);
                            scheduledClasses.push(segment);
                            ...
                            unscheduledClasses.push(segment);
                ...
                for (const classItem of finalClassesToSchedule) {
                    ...
                    console.log('Processing class:', classItem.subject, 'Lecture:', hasLecture, 'Lab:', hasLab, 'lectureHours:', classItem.lectureHours, 'labHours:', classItem.labHours, 'splitLecture:', classItem.splitLectureHours, 'splitLab:', classItem.splitLabHours);
                    ...
                    if (hasLecture && hasLab) {
                        console.log('Class has both lecture and lab - scheduling each component (with split support)');
                        const subjectUnits = parseFloat(classItem.units || classItem.unitLoad || 0);
                        
                        const lectureClass = {
                            ...classItem,
                            id: classItem.id + '-lec',
                            unitLoad: lectureHours || subjectUnits || classItem.unitLoad || 1,
                            units: subjectUnits,
                            classType: 'lecture',
                            labHours: 0
                        };
                        ...
                        const labClass = {
                            ...classItem,
                            id: classItem.id + '-lab',
                            unitLoad: labHours || classItem.unitLoad || 1,
                            units: 0,
                            classType: 'laboratory',
                            lectureHours: 0
                        };
                        
                        const lectureSegments = createSessionsForClassType(lectureClass, 'lecture');
                        const labSegments = createSessionsForClassType(labClass, 'laboratory');
                        const lectureSchedule = scheduleSegments(lectureSegments, subjectScheduledTime, {
                            labelBuilder: (segment, index, total, nextCount, totalTotal) => `Scheduling lecture ${segment.subject} (${index + 1}/${total}) - ${nextCount} of ${totalTotal}`
                        });
                        
                        if (!lectureSchedule.success) {
                            console.warn('Lecture could not be scheduled for', classItem.subject);
                            labSegments.forEach(seg => unscheduledClasses.push(seg));
                            continue;
                        }
                        
                        if (lectureSchedule.results.length > 0 && lectureSchedule.results[0].day && lectureSchedule.results[0].time) {
                            subjectScheduledTimes[subjectKey] = {
                                day: lectureSchedule.results[0].day,
                                time: lectureSchedule.results[0].time
                            };
                        }
                        
                        const lectureAnchor = subjectScheduledTimes[subjectKey];
                        const labFirstContext = lectureAnchor ? { time: lectureAnchor.time, excludeDay: lectureAnchor.day } : subjectScheduledTime;
                        const labSchedule = scheduleSegments(labSegments, labFirstContext, {
                            labelBuilder: (segment, index, total, nextCount, totalTotal) => `Scheduling lab ${segment.subject} (${index + 1}/${total}) - ${nextCount} of ${totalTotal}`
                        });
                        
                        if (!labSchedule.success) {
                            console.warn('Lab could not be scheduled for', classItem.subject);
                        }
                    } else {
                        const sessionType = hasLecture ? 'lecture' : (hasLab ? 'laboratory' : classItem.classType);
                        let segments = [];
                        
                        if (sessionType === 'lecture' || sessionType === 'laboratory') {
                            segments = createSessionsForClassType(classItem, sessionType);
                        }
                        
                        if (!segments || segments.length === 0) {
                            const fallbackSegment = { ...classItem };
                            fallbackSegment.unitLoad = normalizeHours(classItem.unitLoad) || 1;
                            segments = [fallbackSegment];
                        }
                        
                        const scheduleResult = scheduleSegments(segments, subjectScheduledTime, {
                            labelBuilder: (segment, index, total, nextCount, totalTotal) => {
                                if (segment.splitSessionEnabled && total > 1) {
                                    return `Scheduling ${segment.subject} ${segment.splitSessionType || sessionType} part ${index + 1}/${total} - ${nextCount} of ${totalTotal}`;
                                }
                                return `Scheduling class ${nextCount} of ${totalTotal}: ${segment.subject}`;
                            }
                        });
                        
                        if (scheduleResult.success && scheduleResult.results.length > 0) {
                            const firstResult = scheduleResult.results[0];
                            if (firstResult.day && firstResult.time) {
                                subjectScheduledTimes[subjectKey] = {
                                    day: firstResult.day,
    ...
    function scheduleClass(classItem, subjectScheduledTime = null) {
        ...
        // Allow multiple instances of the same class to be scheduled
        // (Removed duplicate check to allow dragging same class multiple times)
        // Users can manually schedule the same class multiple times if needed
        
        // Allow enforcing minimum gap days when scheduling split sessions
        const dayIndexMap = {
            'Monday': 0,
            'Tuesday': 1,
            'Wednesday': 2,
        ...
        if (subjectScheduledTime && subjectScheduledTime.day && subjectScheduledTime.minGapDays) {
            const referenceIndex = dayIndexMap[subjectScheduledTime.day];
            if (typeof referenceIndex === 'number') {
                possibleDays = possibleDays.filter(d => {
                    const candidateIndex = dayIndexMap[d.id];
                    if (typeof candidateIndex !== 'number') return true;
                    return Math.abs(candidateIndex - referenceIndex) >= subjectScheduledTime.minGapDays;
                });
            }
        }
    ...
    function saveClassesToLocalStorage() {
        try {
            ...
            // If classes were explicitly cleared and we're trying to save an empty array, that's fine
            // But if we're trying to save classes when they were cleared, respect the cleared state
            const classesClearedFlag = localStorage.getItem('classesCleared');
            if (classesClearedFlag && classesToSave.length === 0) {
                // Explicitly cleared and empty - ensure it stays empty
                localStorage.setItem('allClasses', JSON.stringify([]));
                console.log('Classes were cleared - keeping empty state');
                return;
            }
            
            // If we have classes to save, remove the cleared flag
            if (classesToSave.length > 0 && classesClearedFlag) {
                localStorage.removeItem('classesCleared');
            }
    ...
    async function loadClassesFromLocalStorage() {
        try {
            const classesClearedFlag = localStorage.getItem('classesCleared');
            let savedClasses = JSON.parse(localStorage.getItem('allClasses') || '[]');
            
            // If classes were explicitly cleared, treat as empty regardless of stale data
            if (classesClearedFlag) {
                if (savedClasses && savedClasses.length) {
                    console.log('Classes were cleared previously. Ignoring stale saved classes.');
                    // Force storage to empty array
                    localStorage.setItem('allClasses', JSON.stringify([]));
                }
                savedClasses = [];
            }
            ...
            if (savedClasses && savedClasses.length) {
                ...
                // Since we have classes again, remove any "cleared" flag
                localStorage.removeItem('classesCleared');
            } else {
    ...
    document.addEventListener('visibilitychange', function() {
        if (!document.hidden) {
            // Check if classes were explicitly cleared - if so, don't reload
            const classesClearedFlag = localStorage.getItem('classesCleared');
            if (classesClearedFlag) {
                console.log('Classes were cleared, skipping reload on visibility change');
                return;
            }
    ...
    window.addEventListener('focus', function() {
        ...
        const classesClearedFlag = localStorage.getItem('classesCleared');
        if (classesClearedFlag) {
            console.log('Classes were cleared, skipping reload on focus');
            return;
        }
    ...
    function initializeFilters() {
        ...
        // Function to populate filter options from calendar events
        function populateFilterOptions() {
            const calendar = window.calendar;
            if (!calendar || typeof calendar.getEvents !== 'function') {
                console.warn('Calendar not available for filter initialization');
                return;
            }
        ...
        function applyFilters() {
            ...
            events.forEach(event => {
                ...
                if (selectedFaculty && props.faculty !== selectedFaculty) shouldShow = false;
                if (selectedRoom && props.room !== selectedRoom) shouldShow = false;
                if (selectedCourse && props.course !== selectedCourse) shouldShow = false;
            ...
            if (filterStatus) {
                if (selectedFaculty || selectedRoom || selectedCourse) {
                    filterStatus.textContent = `Showing ${visibleCount} of ${events.length} schedules`;
                } else {
        ...
        // Populate filters when calendar is ready
        if (window.calendar) {
        ...
        // Re-populate filters when events are added/removed
        const originalAddEvent = window.calendar?.addEvent;
        if (originalAddEvent && window.calendar) {
            window.calendar.addEvent = function(...args) {
                const result = originalAddEvent.apply(this, args);
    ...
    // Initialize filters after a short delay to ensure calendar is ready
    setTimeout(initializeFilters, 500);
    
    // Function to validate faculty unit limits
    async function validateFacultyUnitLimit(facultyId, newUnitLoad) {
        try {
            console.log('=== Faculty Unit Limit Validation ===');
            console.log('Faculty ID:', facultyId);
            console.log('New Unit Load:', newUnitLoad);
            
            // Get faculty member data - check multiple sources
            let facultyMembers = JSON.parse(localStorage.getItem('facultyMembers') || '[]');
            
            // Also try to get from users data (faculty might be stored there)
            if (facultyMembers.length === 0) {
                const users = JSON.parse(localStorage.getItem('users') || '[]');
                facultyMembers = users.filter(u => u.role === 'faculty' || u.role === 'teacher');
                console.log('Loaded faculty from users data:', facultyMembers.length);
            }
            
            console.log('Total faculty members found:', facultyMembers.length);
            console.log('Faculty members:', facultyMembers);
            
            // Try multiple ways to find the faculty
            let faculty = facultyMembers.find(f => String(f.id) === String(facultyId));
            if (!faculty) {
                // Try matching by email if facultyId might be an email
                faculty = facultyMembers.find(f => String(f.email) === String(facultyId));
            }
            if (!faculty) {
                ...
                if (facultySelect) {
                    ...
                    if (selectedText) {
                        faculty = facultyMembers.find(f => {
                            const fullName = `${f.firstName || ''} ${f.middleName || ''} ${f.lastName || ''}`.trim();
                            return fullName === selectedText || f.email === selectedText;
                        });
                    }
                }
            }
            ...
            if (!faculty) {
                console.warn('Faculty not found in localStorage. Faculty ID:', facultyId);
                console.warn('Available faculty:', facultyMembers.map(f => ({ id: f.id, name: `${f.firstName} ${f.lastName}`, email: f.email, employmentType: f.employmentType })));
                ...
                try {
                    ...
                    if (token) {
                        ...
                        if (response && response.ok) {
                            ...
                            console.log('Found faculty from server:', faculty);
                        }
                    }
                } catch (e) {
                    console.warn('Could not fetch faculty from server:', e);
                }
                
                // If still not found, block to be safe
                if (!faculty) {
                    console.error('Faculty not found - blocking class creation to prevent unit limit violations');
                    return { 
                        allowed: false, 
                        message: `Cannot verify faculty employment type. Please ensure the faculty member is properly registered in the system.`,
            ...
            // Get faculty employment type - check multiple possible field names
            const employmentTypeRaw = faculty.employmentType || faculty.employment_type || faculty.employmentStatus || 'full-time';
            const employmentType = String(employmentTypeRaw).toLowerCase().trim();
            const isPartTime = employmentType.includes('part') || employmentType === 'parttime' || employmentType === 'part-time';
            const mixedTeaching = faculty.mixedTeaching === true || faculty.mixed_teaching === true;
            const departmentId = faculty.departmentId || faculty.department_id || '';
            const facultyName = `${faculty.firstName || ''} ${faculty.middleName || ''} ${faculty.lastName || ''}`.replace(/\s+/g, ' ').trim() || faculty.email || 'Faculty';
            
            console.log('Employment Type (raw):', employmentTypeRaw);
            console.log('Employment Type (normalized):', employmentType);
            console.log('Is Part-Time:', isPartTime);
            console.log('Mixed Teaching:', mixedTeaching);
            
            // Calculate current unit load from existing scheduled events
            const calendar = window.calendar;
            let currentUnits = 0;
            ...
            if (calendar) {
                ...
                events.forEach(event => {
                    const props = event.extendedProps || {};
                    // Check both facultyId and faculty name
                    const matchesFaculty = (props.facultyId && String(props.facultyId) === String(facultyId)) ||
                                          (props.faculty && props.faculty === facultySelect?.options[facultySelect.selectedIndex]?.text);
                    ...
                    if (matchesFaculty) {
                        ...
                        if (props.subjectId) {
                            ...
                            const subject = subjects.find(s => String(s.id) === String(props.subjectId));
                            if (subject && subject.units) {
                                currentUnits += parseFloat(subject.units) || 0;
                                scheduledCount++;
                                unitsFound = true;
                            }
                        }
                        ...
                        if (!unitsFound && props.subject) {
                            const subjects = JSON.parse(localStorage.getItem('subjects') || '[]');
                            const subject = subjects.find(s => 
                                s.name && s.name.toLowerCase() === props.subject.toLowerCase()
                            );
                            if (subject && subject.units) {
                                currentUnits += parseFloat(subject.units) || 0;
                                scheduledCount++;
                        ...
                        if (!unitsFound && props.unitLoad) {
                            currentUnits += parseFloat(props.unitLoad) || 0;
                            scheduledCount++;
                        }
                    }
                });
            ...
            console.log('Units from scheduled events:', currentUnits, '(', scheduledCount, 'events)');
            
            // Also check from allClasses array (classes not yet scheduled)
            let pendingCount = 0;
            let pendingUnits = 0;
            if (window.allClasses && Array.isArray(window.allClasses)) {
                window.allClasses.forEach(classItem => {
                    // Check both facultyId and faculty name
                    const matchesFaculty = (classItem.facultyId && String(classItem.facultyId) === String(facultyId)) ||
                                         (classItem.faculty && classItem.faculty === facultySelect?.options[facultySelect.selectedIndex]?.text);
                    ...
                    if (matchesFaculty) {
                        ...
                        if (classItem.subjectId) {
                            const subjects = JSON.parse(localStorage.getItem('subjects') || '[]');
                            const subject = subjects.find(s => String(s.id) === String(classItem.subjectId));
                            if (subject && subject.units) {
                                pendingUnits += parseFloat(subject.units) || 0;
                                pendingCount++;
                                unitsFound = true;
                            }
                        }
                        ...
                        // If not found by subjectId, try to find by subject name
                        if (!unitsFound && classItem.subject) {
                            const subjects = JSON.parse(localStorage.getItem('subjects') || '[]');
                            const subject = subjects.find(s => 
                                s.name && s.name.toLowerCase() === classItem.subject.toLowerCase()
                            );
                            if (subject && subject.units) {
                                pendingUnits += parseFloat(subject.units) || 0;
                                pendingCount++;
                        ...
                        // Fallback to units or unitLoad if no subject units found
                        if (!unitsFound) {
                            if (classItem.units) {
                                pendingUnits += parseFloat(classItem.units) || 0;
                                pendingCount++;
                            } else if (classItem.unitLoad) {
                                pendingUnits += parseFloat(classItem.unitLoad) || 0;
                                pendingCount++;
                            }
                        }
                    }
            ...
            currentUnits += pendingUnits;
            console.log('Units from pending classes:', pendingUnits, '(', pendingCount, 'classes)');
            console.log('Total current units:', currentUnits);
            
            const totalUnits = currentUnits + newUnitLoad;
            console.log('Total units after adding new class:', totalUnits);
            
            // Determine unit limits based on employment type and department
            let baseLimit = 24; // Default for full-time tertiary
            let absoluteLimit = 24;
            let facultyType = 'Tertiary (Full-time)';
            let canRequestOverride = false;
            
            if (isPartTime) {
                baseLimit = 15;
                absoluteLimit = 15;
                facultyType = 'Part-time';
                canRequestOverride = false; // Part-time can NEVER exceed 15 units
                console.log('Part-time faculty detected - hard limit of 15 units enforced');
            } else {
                // Check if SHS department
                const departments = JSON.parse(localStorage.getItem('departments') || '[]');
                const department = departments.find(d => String(d.id) === String(departmentId) || String(d.code) === String(departmentId));
                
                const isSHS = department && (
                    (department.code && department.code.toUpperCase().includes('SHS')) ||
                    (department.code && ['STEM','ABM','HUMSS','GAS'].some(code => department.code.toUpperCase().includes(code))) ||
                    department.name?.toLowerCase().includes('senior high') ||
                    department.name?.toLowerCase().includes('shs')
                );
                
                if (isSHS) {
                    baseLimit = 27;
                    absoluteLimit = 27;
                    facultyType = 'SHS (Full-time)';
                } else {
                    baseLimit = 24;
                    absoluteLimit = 24;
                    facultyType = 'Tertiary (Full-time)';
            ...
            console.log('Absolute Limit:', absoluteLimit);
            console.log('Can Request Override:', canRequestOverride);
            
            // Hard limit check - especially important for part-time faculty
            if (totalUnits > absoluteLimit) {
                let message = '';
                if (isPartTime) {
                    message = `PART-TIME FACULTY UNIT LIMIT EXCEEDED!\n\n` +
                        `Faculty Member: ${facultyName}\n` +
                        `Employment Type: Part-time\n` +
                        `Current Units: ${currentUnits.toFixed(1)}\n` +
                        `Adding: ${newUnitLoad.toFixed(1)} units\n` +
                        `Total Would Be: ${totalUnits.toFixed(1)} units\n\n` +
                        `Part-time faculty members have a HARD LIMIT of ${absoluteLimit} units and CANNOT exceed this limit.\n` +
                        `Please reduce the unit load or select a different faculty member.`;
                } else {
                    message = `${facultyType} FACULTY UNIT LIMIT EXCEEDED!\n\n` +
                        `Faculty Member: ${facultyName}\n` +
                        `Employment Type: Full-time\n` +
                        `Current Units: ${currentUnits.toFixed(1)}\n` +
                        `Adding: ${newUnitLoad.toFixed(1)} units\n` +
                        `Total Would Be: ${totalUnits.toFixed(1)} units\n\n` +
                        `Maximum allowed: ${absoluteLimit} units.\n` +
                        `Please reduce the unit load or select a different faculty member.`;
                }
                console.error('VALIDATION FAILED (absolute limit):', message);
                return {
                    allowed: false,
                    message,
                    facultyName,
            ...
            // Base limit check with optional override
            if (totalUnits > baseLimit) {
                const message = `${facultyType} faculty base limit is ${baseLimit} units. ` +
                    `Current: ${currentUnits.toFixed(1)}, Adding: ${newUnitLoad.toFixed(1)}, Total: ${totalUnits.toFixed(1)}.`;
                
                if (canRequestOverride) {
                    console.warn('Base limit exceeded but eligible for override. Awaiting approval.');
                    return {
                        allowed: false,
                        message,
                        facultyName,
                        ...
                        mixedTeaching: mixedTeaching
                    };
                }
                
                // Improve message to include employment type
                const improvedMessage = `${facultyType} FACULTY BASE LIMIT EXCEEDED!\n\n` +
                    `Faculty Member: ${facultyName}\n` +
                    `Employment Type: ${isPartTime ? 'Part-time' : 'Full-time'}\n` +
                    `Current Units: ${currentUnits.toFixed(1)}\n` +
                    `Adding: ${newUnitLoad.toFixed(1)} units\n` +
                    `Total Would Be: ${totalUnits.toFixed(1)} units\n\n` +
                    `Base limit: ${baseLimit} units.\n` +
                    `Please reduce the unit load or select a different faculty member.`;
                
                console.error('VALIDATION FAILED (base limit, no override):', improvedMessage);
                return {
                    allowed: false,
            ...
            // FINAL SAFETY CHECK: If faculty is part-time, absolutely block anything over 15 units
            // This is a hard requirement that cannot be overridden
            if (isPartTime && totalUnits > 15) {
                const safetyMessage = `PART-TIME FACULTY UNIT LIMIT EXCEEDED!\n\n` +
                    `Faculty Member: ${facultyName}\n` +
                    `Employment Type: Part-time\n` +
                    `Current Units: ${currentUnits.toFixed(1)}\n` +
                    `Adding: ${newUnitLoad.toFixed(1)} units\n` +
                    `Total Would Be: ${totalUnits.toFixed(1)} units\n\n` +
                    `Part-time faculty members have a HARD LIMIT of 15 units and CANNOT exceed this limit.\n` +
                    `This limit cannot be overridden, even by superadmins.\n` +
                    `Please reduce the unit load or select a different faculty member.`;
                console.error('FINAL SAFETY CHECK FAILED - Part-time faculty exceeds 15 units:', safetyMessage);
                return {
                    allowed: false,
            ...
            console.error('Error validating faculty unit limit:', error);
            console.error('Stack:', error.stack);
            return { allowed: true, message: '' }; // Allow on error to not block functionality
        }
    }
    
    // Expose function globally for use in other parts of the code
    window.validateFacultyUnitLimit = validateFacultyUnitLimit;
});
        ...
        clearClassesBtn.addEventListener('click', function() {
            ...
            if (clearModal) {
                ...
                newConfirmBtn.addEventListener('click', async function() {
                    // Clear all classes - await the async function
                    if (typeof clearAllClasses === 'function') {
                        await clearAllClasses(classesList, classItems);
                    } else {
                        // Fallback clearing - must also clear localStorage
                        classItems.forEach(item => item.remove());
                        allClasses = [];
                        window.allClasses = [];
                    ...
                    // Verify classes were actually cleared before closing modal
                    const verifyAfterClear = JSON.parse(localStorage.getItem('allClasses') || '[]');
                    if (verifyAfterClear.length === 0) {
                        console.log('✓ Classes successfully cleared and verified');
                    } else {
                        console.error('✗ ERROR: Classes still exist after clear!', verifyAfterClear);
